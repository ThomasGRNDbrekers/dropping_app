import streamlit as st
import pandas as pd
import gspread
from google.oauth2.service_account import Credentials
import folium
from streamlit_folium import st_folium
from math import radians, cos, sin, asin, sqrt
import re

# --- 1. CONFIGURATIE ---
st.set_page_config(page_title="Dropping 2026", layout="wide")

RAW_KEY = r"-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----"

def fix_key(key):
    return key.replace(r'\n', '\n').replace('\\n', '\n').strip()

SERVICE_ACCOUNT_INFO = {
    "type": "service_account",
    "project_id": "dropping2026",
    "private_key": fix_key(RAW_KEY),
    "client_email": "droping-final@dropping2026.iam.gserviceaccount.com",
    "token_uri": "https://oauth2.googleapis.com/token",
}

@st.cache_resource
def get_ss_worksheet():
    scopes = ["https://www.googleapis.com/auth/spreadsheets", "https://www.googleapis.com/auth/drive"]
    creds = Credentials.from_service_account_info(SERVICE_ACCOUNT_INFO, scopes=scopes)
    client = gspread.authorize(creds)
    sh = client.open_by_key("13KipcWXoXnf-ZRK_sughyft3qYOEoYlSf9XAj_dE9kI")
    return sh.worksheet("Data")

# --- 2. ROBUUSTE DATA FUNCTIES ---
def get_db():
    ws = get_ss_worksheet()
    data = ws.get_all_records()
    if not data:
        # Als de sheet leeg is, maken we een lege DataFrame met de juiste kolommen
        return pd.DataFrame(columns=["Teamnaam", "Leden", "Fase", "Alarm", "Score", "Huidige_Lat", "Huidige_Lon"])
    return pd.DataFrame(data)

def save_to_db(df):
    ws = get_ss_worksheet()
    ws.clear()
    # Zorg dat de headers ALTIJD geschreven worden
    ws.update([df.columns.values.tolist()] + df.values.tolist())

# --- 3. UI LOGICA ---
if 'logged_in' not in st.session_state:
    st.session_state.logged_in = False

if not st.session_state.logged_in:
    st.title("📍 Dropping 2026")
    team_input = st.text_input("Teamnaam").strip()
    leden_input = st.text_input("Namen leden")
    
    if st.button("Start"):
        if team_input == "THOMASBAAS":
            st.session_state.role = "admin"
            st.session_state.logged_in = True
            st.rerun()
        elif team_input:
            df = get_db()
            # Controleer of 'Teamnaam' kolom bestaat, anders toevoegen
            if "Teamnaam" not in df.columns:
                 df = pd.DataFrame(columns=["Teamnaam", "Leden", "Fase", "Alarm", "Score", "Huidige_Lat", "Huidige_Lon"])
            
            if team_input not in df['Teamnaam'].values:
                new_row = {"Teamnaam": team_input, "Leden": leden_input, "Fase": "START", "Alarm": "GEEN", "Score": 0, "Huidige_Lat": 0.0, "Huidige_Lon": 0.0}
                df = pd.concat([df, pd.DataFrame([new_row])], ignore_index=True)
                save_to_db(df)
            
            st.session_state.team = team_input
            st.session_state.role = "user"
            st.session_state.logged_in = True
            st.rerun()
else:
    # --- ADMIN ---
    if st.session_state.role == "admin":
        st.title("🕹️ Control Room")
        df = get_db()
        st.dataframe(df)
        
        if not df.empty and "Teamnaam" in df.columns:
            target = st.selectbox("Stuur bericht naar:", df['Teamnaam'].unique())
            msg = st.text_input("Alarm bericht:")
            if st.button("Verstuur"):
                df.loc[df['Teamnaam'] == target, 'Alarm'] = msg
                save_to_db(df)
                st.success("Verzonden!")
        else:
            st.info("Nog geen teams aangemeld.")
            
        if st.button("Log uit"):
            st.session_state.logged_in = False
            st.rerun()

    # --- USER ---
    else:
        st.title(f"Team: {st.session_state.team}")
        df = get_db()
        # Zoek data van dit team
        my_data = df[df['Teamnaam'] == st.session_state.team].iloc[0]
        
        if my_data['Alarm'] != "GEEN":
            st.error(f"🚨 BERICHT: {my_data['Alarm']}")
            if st.button("Gelezen"):
                df.loc[df['Teamnaam'] == st.session_state.team, 'Alarm'] = "GEEN"
                save_to_db(df)
                st.rerun()

        st.metric("Score", f"{my_data['Score']} pnt")
        if st.button("Log uit"):
            st.session_state.logged_in = False
            st.rerun()
