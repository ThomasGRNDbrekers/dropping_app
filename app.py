...
# Deze JS zorgt ervoor dat je teamnaam in de browser blijft staan, zelfs na een refresh
st.components.v1.html("""
<script>
    const team = localStorage.getItem('dropping_team');
    const role = localStorage.getItem('dropping_role');
    if (team && !window.parent.location.href.includes('auth=true')) {
        window.parent.postMessage({type: 'streamlit:set_component_value', value: {team: team, role: role}, key: 'ls_login'}, '*');
    }
</script>
""", height=0)
...
START_PUNTEN = 1000.0
PUNTEN_PER_SEC = START_PUNTEN / (5 * 3600) # 5 uur doel
...
def get_db_as_df():
    ...
    df = pd.DataFrame(data if data else columns=EXPECTED_COLS)
...
def save_df_to_db(df):
    ws = get_ss_worksheet()
    if ws: ws.update([df.columns.values.tolist()] + df.values.astype(str).tolist())

# --- 3. DASHBOARDS ---

@st.fragment(run_every=10) # Controller ververst elke 10 sec
def show_admin_dashboard():
    ...
    with col1:
        st.subheader("📍 Live Kaart Alle Teams")
    ...
    with col2:
        ...
        st.subheader("🚀 Alarm Pushen")
        target = st.selectbox("Kies Team:", df['Teamnaam'].unique() if not df.empty else ["Geen teams"])
        msg = st.text_input("Opdracht tekst")
        mins = st.number_input("Tijdslimiet (minuten)", 10)
        if st.button("Push Alarm naar Team"):
            deadline = time.time() + (mins * 60)
            df.loc[df['Teamnaam'] == target, 'Alarm'] = f"50|{msg}|{deadline}"
            save_df_to_db(df)
            st.success(f"Alarm gestuurd naar {target}!")

@st.fragment(run_every=5) # Deelnemer ververst elke 5 sec voor GPS & Klok
def show_user_dashboard():
    # Forceer GPS update via JS
    st.components.v1.html("""
    <script>
    ...
    if my_data['Fase'] == "LOCATIE_KIEZEN":
        st.info("Duid op de kaart aan waar je bent gedropt om te starten.")
        ...
        # Blind Mode logica (na 5 meter wandelen)
        ...
        folium.PolyLine([[my_data['Start_Lat'], my_data['Start_Lon']], FINISH_COORDS], color="green", weight=5, opacity=0.8).add_to(m)
...
if not st.session_state.logged_in:
    ...
    if st.button("Inloggen"):
        ...
        # JS om login op te slaan in browser
        st.components.v1.html(f"<script>localStorage.setItem('dropping_team', '{name}'); localStorage.setItem('dropping_role', '{role}'); window.parent.location.reload();</script>", height=0)
        st.rerun()
else:
